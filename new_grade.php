<?php 
	...
	if(count($errors) === 0){
		...
		if($_POST['grade_id'] == ""){
			$sql = "INSERT INTO grade (`from`, `to`, `grade`, `comment`) VALUES ('$from', '$to', '$grade', '$comment')";
			$sql2 = "INSERT INTO history_log (`transaction`, `user_id`, `date_added`) VALUES ('added $grade in the grades list', '$user', NOW())";
			...
			if (mysqli_query($conn, $sql) && mysqli_query($conn, $sql2)) {
				...
				echo "<script>
				alert('New Grade failed to record! Error: " . mysqli_error($conn) . "');
			...
			$sql2 = "INSERT INTO history_log (`transaction`, `user_id`, `date_added`) VALUES ('updated $id in the grades list', '$user', NOW())";
	...
	// For debugging purposes, you might want to log errors to a file
	// error_log("SQL Error: " . mysqli_error($conn), 3, "/var/log/php_errors.log");
